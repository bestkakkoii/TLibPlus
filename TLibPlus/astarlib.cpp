...
AStar::AStar(BlockAllocator* allocator)
	...
	, heuristictype_(Heuristic::manhattan)
{
...
void AStar::set_heuristic_type(const Heuristic& type)
{
	heuristictype_ = type;
}
...
{
	uint16_t g_value = current.distance(heuristictype_, parent->pos) == 2 ? oblique_val_ : step_val_;
...
{
	unsigned int h_value = end.distance(heuristictype_, current);
...
bool AStar::can_pass(const Vec2& current, const Vec2& destination, bool allow_corner)
{
	if (destination.x >= 0 && destination.x < width_ && destination.y >= 0 && destination.y < height_)
	{
		...
		if (destination.distance(heuristictype_, current) == 1)
		{
